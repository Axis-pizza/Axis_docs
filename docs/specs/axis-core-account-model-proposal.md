...
> ステータス: **approval-ready proposal analysis**（承認用提案分析）。本書は最終的な Rust 実装仕様ではありません。account layout、field の bit 幅、instruction account ordering、PDA seed の最終形は engineering review 対象です（`requirements/19-axis-core-implementation-rfc.md` §11, §12）。
>
...
本提案は account-model（状態所有・権限・lifecycle・invariant）の提案であり、instruction surface 提案・CPI 実装・数式確定は別 spike / 別 spec の対象です。

---

## 3. Non-goals / 対象外

- Rust 実装、contract repository への変更（本タスクで明示的に禁止）。
...
_本提案は approval-ready proposal analysis であり、Rust 実装・contract repo 変更・既存 requirement 変更は含みません。canonical 値はすべて Axis_docs requirement ID に紐付け、未確定事項は review question として明示しています。_
